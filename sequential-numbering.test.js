...
assert.match(liveHtml, /data-original-number="899"/);
...
assert.deepEqual(reportRows.map((row) => row.questionId), expectedOrder);
assert.deepEqual(reportRows.map((row) => row.displayNumber), Array.from({ length: 50 }, (_, index) => index + 1));
assert.equal(reportRows[2].originalNumber, 881);
