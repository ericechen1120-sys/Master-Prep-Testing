...
If the logo is missing, confirm that both `assets/topway-prep-logo.png` and `assets/topway-prep-logo-transparent.png` were uploaded, and that `assets/` is at the repository root beside `app.js` and `server.js`. The student exam header uses the transparent file. Opening the public logo URL should return the image, not `Not found`.
