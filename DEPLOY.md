...
The student exam header uses a built-in text wordmark, so it remains complete even if an image asset has not deployed yet. For reports and the admin interface, confirm that both `assets/topway-prep-logo.png` and `assets/topway-prep-logo-transparent.png` are at the repository root beside `app.js` and `server.js`. Opening either public logo URL should return the image, not `Not found`.
