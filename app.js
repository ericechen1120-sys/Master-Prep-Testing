      ...
      <main class="locked login-stage">
        <section class="login-showcase">
          <div class="login-hero">
            ...
            <p>Please enter your name and assigned exam code to begin the exam.</p>
          </div>
    ...
    <div class="grid two">
      <div class="csv-import-box">
        <div class="field">
          ...
          <p class="hint" id="bankCsvStatus">Import keeps questions editable, including Question Image URL, Graph URL, and Passage Image URL columns.</p>
        </div>
        <div class="field">
          <label>System question ID prefix</label>
  ...
  return `
    <div class="card stack">
      ...
      <div class="report-print-actions no-print">
        ${submission ? `<button class="ghost" onclick="printSelectedReport()">Print Selected Report</button>` : `<button class="ghost" disabled>Select a report first</button>`}
      ...
      <p class="hint">Use the Results subtab to choose a specific student report first.</p>
    </div>
  `;
}
...
function renderClassDetail(classRecord) {
  ...
  return `
    ...
    ${renderClassDetailSubtab(classRecord, roster, reports, attempts, linkedExamIds)}
  `;
}
...
function renderQuestionLivePreview(question) {
  ...
  return `
    ...
    <article class="question preview-question" data-section="${escapeHtml(draft.section)}" data-original-number="${draft.originalNumber || draft.number}">
      ...
      ${renderQuestionTextWithInlineMedia(draft, { preview: true, mediaPrefix: "preview-question" })}
...
function printSelectedDiagnosticReport() {
  ...
  $("#app").innerHTML = `
    <main class="report-only stack">
      ...
      ${renderDiagnosticReport(submission, { branded: true })}
    </main>
  `;
}
...
function renderDiagnosticReport(submission, options = {}) {
  ...
  return `
    <article class="diagnostic-report ${branded ? "branded-report" : ""}">
...
function analyzeQuestionCsv(text) {
  ...
  rows.slice(1).forEach((row, index) => {
    ...
    if (summary.examples.length < 3) summary.examples.push({ number: index + 1, question, skill, difficulty, type: isGrid ? "Grid-in" : "Multiple choice", graphics: hasQuestionImage || hasSharedImage || inlineImages.length });
...
function renderStudentQuestion(question, displayNumber, hideSharedMedia = false, showOwnPassage = false) {
  ...
  return `
    <article class="question" data-section="${escapeHtml(question.section)}" data-original-number="${question.originalNumber || question.number}">
      ...
      ${renderQuestionTextWithInlineMedia(question, { mediaPrefix: "student-question" })}
