...
function selectPreferredBankQuestionVersions(questions) {
  ...
  return [...(questions || [])]
    .sort((left, right) => {
      ...
      return leftGenerated - rightGenerated || bankQuestionUiSort(left, right);
    })
    .filter((question) => {
...
function buildSmartBankExamDraft() {
  ...
  anchorPool.forEach((anchor) => {
    ...
    if (hasWrongSubject || hasWrongSource || hasInvalidQuestion || hasUsedQuestion || passageTooLarge) {
  ...
  const canAddUnit = (unit) => {
    ...
    for (const question of unit.questions) {
      ...
      if (avoidDuplicates && signature && (duplicateSignatures.has(signature) || unitSignatures.has(signature))) return false;
