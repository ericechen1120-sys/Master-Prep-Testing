...
const QUESTION_TYPE_OPTIONS = [
  ...
  ["table_grid", "Claim / evidence table"],
];
...
function normalizeQuestionType(value) {
  ...
  if (/grid|numeric|free|short|equation/.test(type)) return "numeric";
...
function renderQuestionTextWithInlineMedia(question, options = {}) {
  ...
  const inlineImages = collectQuestionTextImages(text);
  const fontStyle = `font-family: ${escapeHtml(getQuestionFontCss(question))}`;
  const textHtml = cleanText ? `<div class="question-text" style="${fontStyle}">${formatRichText(cleanText)}</div>` : preview && !inlineImages.length ? `<div class="notice compact">No question text yet.</div>` : "";
...
function renderAdminQuickActions() {
  const actionsByTab = {
    ...
    bank: [
      ...
      { label: "Cleanup", tab: "bank", subtab: "cleanup" },
    ],
    students: [
    ...
    tools: [
      ...
      { label: "Calculator", tab: "tools", subtab: "calculator" },
    ],
  };
...
function getAdminSubTab(tab = adminActiveTab) {
  return adminSubTabs[tab] || getAdminSubTabItems(tab)[0]?.id || "";
}
  ...
  return (
    {
      ...
      tools: [
        ...
        { id: "calculator", label: "Calculator", meta: "SAT" },
      ],
    }[tab] || []
  );
}
  ...
  return `
    <section class="panel stack">
      ...
      <div class="row">
        ...
        <label class="row" style="margin-top: 22px"><input id="examShsatOriginal" type="checkbox" /> Use Original SHSAT raw-score conversion (exactly 50 questions)</label>
      </div>
      <div class="field">
  ...
  return `
    <section class="admin-workspace stack">
      ...
      <div class="panel stack">
        ...
        ${subtab === "formats" ? `${renderBankFilters(bankClasses, skills, difficulties)}${renderAiFormatAssistant(questions)}` : ""}
        ...
        ${subtab === "edit" ? `${renderBankFilters(bankClasses, skills, difficulties)}${renderBankBatchFixPanel(questions)}${renderBankQuestionEditor(questions, selectedQuestion)}` : ""}
        ${subtab === "cleanup" ? `${renderBankFilters(bankClasses, skills, difficulties)}${renderBankIdFixPanel(idHealth, questions)}${renderBankDuplicateDetector(questions)}` : ""}
    ...
    <div class="grid two">
      <div class="csv-import-box">
        <div class="field">
          ...
          <p class="hint" id="bankCsvStatus">Use Question Type: multiple, numeric, fill_blank, dropdown, drag_drop, or table_grid. Use Options for dropdowns, drag items, and table columns. Drag questions also need Drop Zones; claim/evidence tables need Grid Rows and an answer mapping such as 1=Incorrect; 2=Correct.</p>
        </div>
        <div class="field">
          <label>System question ID prefix</label>
  ...
  return `
    <div class="grid two">
      <div class="card stack">
        ...
        <label class="row"><input id="bankExamShsatOriginal" type="checkbox" /> Use Original SHSAT raw-score conversion (exactly 50 questions)</label>
        ...
        <p class="hint">Use filters for a full set, or check exact questions in Edit for a custom exam.</p>
      ...
      <div class="card stack question-mix-builder">
        ...
        <div class="bank-selection-actions">
          <button class="ghost" onclick="applyShsatEla50Mix()" type="button">Use SHSAT ELA 50 Mix</button>
        ...
        <p class="hint">SHSAT ELA 50 Mix: 45 multiple choice, 1 fill-in-the-blank, 1 dropdown, 1 drag-and-drop, and 2 claim/evidence tables. The system clearly flags any missing question type; it never invents questions.</p>
      </div>
      <div class="card stack">
...
function getFilteredBankQuestions() {
  ...
  return [...(state.questionBank || [])]
    .filter((question) => {
      ...
      const matchesType = bankTypeFilter === "all" || normalizeQuestionType(question.type) === bankTypeFilter;
      return matchesSearch && matchesClass && matchesSkill && matchesDifficulty && matchesType;
    })
...
function selectQuestionMixCandidate(pool, selected, signatures, type = "") {
  return pool
...
function buildQuestionMixDraft() {
  ...
  const pool = getFilteredBankQuestions().filter((question) => question.answer && (question.questionText || question.imageUrl));
  const selected = [];
  const signatures = new Set();
  const shortfalls = [];
  Object.entries(requested).forEach(([type, count]) => {
    for (let index = 0; index < count; index += 1) {
      const candidate = selectQuestionMixCandidate(pool, selected, signatures, type);
      if (!candidate) {
  ...
  for (let index = 0; index < flexible; index += 1) {
    const candidate = selectQuestionMixCandidate(pool, selected, signatures);
    if (!candidate) {
...
function getAllExactTextDuplicateExtraIds(questions = getFilteredBankQuestions()) {
  ...
  return [...new Set(scan.exactText.flatMap(getDuplicateExtraIds))];
}
...
function renderAiFormatAssistant(questions) {
  const selectedIds = getCheckedBankQuestionIds();
  const selectedQuestions = (state.questionBank || []).filter((question) => selectedIds.includes(question.id));
  const mcqCount = selectedQuestions.filter((question) => normalizeQuestionType(question.type) === "multiple").length;
  return `
    <section class="card stack bank-ai-card">
      <div class="section-head">
        <div>
          <h3>AI Format Assistant</h3>
          <p class="subtle">Turn checked MCQ bank questions into review drafts for different student interaction styles.</p>
        </div>
        <span class="pill warn">Review before save</span>
      </div>
      <div class="notice compact">First open <strong>Edit</strong>, check the original MCQ questions you want to convert, then return here. The original question is never changed and no draft is added to an exam automatically.</div>
      <div class="csv-health">
        <span class="pill ${mcqCount ? "ok" : "warn"}">${mcqCount} MCQ${mcqCount === 1 ? "" : "s"} checked</span>
        <span class="pill">${questions.length} in current filter</span>
        ${selectedQuestions.length !== mcqCount ? `<span class="pill warn">${selectedQuestions.length - mcqCount} non-MCQ ignored</span>` : ""}
      </div>
      <div class="grid two compact-grid">
        <label class="row"><input id="aiFormatFillBlank" type="checkbox" checked /> Fill in the blank</label>
        <label class="row"><input id="aiFormatDropdown" type="checkbox" checked /> Inline dropdown</label>
        <label class="row"><input id="aiFormatDragDrop" type="checkbox" /> Drag and drop</label>
        <label class="row"><input id="aiFormatTableGrid" type="checkbox" /> Claim / evidence table</label>
      </div>
      <div class="bank-selection-actions">
        <button class="primary" onclick="generateAiFormatDrafts(event)" type="button">Generate Review Drafts</button>
        <button class="ghost" onclick="openAiFormatSourceSelection()" type="button">Choose MCQs in Edit</button>
      </div>
      <p class="hint">For accuracy and cost control, generate up to 12 checked MCQs at a time. AI will omit a format it cannot create without guessing. Review wording, choices, and every answer mapping before you save a draft.</p>
      ${renderAiFormatDrafts()}
    </section>
  `;
}

function renderAiFormatDrafts() {
  if (!formatAssistantDrafts.length) return `<div class="notice compact">No AI drafts yet. Nothing has been added to your question bank.</div>`;
  return `
    <div class="stack" data-ai-format-drafts>
      <div class="section-head"><div><h3>Review drafts</h3><p class="subtle">Save only the drafts you approve. Saved drafts are marked as AI-generated in the Bank.</p></div><span class="pill">${formatAssistantDrafts.length} ready for review</span></div>
      ${formatAssistantDrafts
        .map((draft, index) => {
          const candidate = draft.candidate || {};
          const mapping = ["drag_drop", "table_grid"].includes(candidate.type) ? `<pre class="ai-draft-mapping">${escapeHtml(candidate.answer || "")}</pre>` : "";
          return `
            <article class="card stack ai-format-draft">
              <div class="csv-health"><span class="pill">Source: ${escapeHtml(draft.sourceSystemId || "MCQ")}</span><span class="pill ok">${escapeHtml(questionTypeLabel(candidate.type))}</span><span class="pill warn">Not saved</span></div>
              <div class="question-text">${formatRichText(candidate.questionText || "")}</div>
              ${candidate.choices?.length ? `<p class="hint"><strong>Options/items:</strong> ${escapeHtml(candidate.choices.join(" · "))}</p>` : ""}
              ${candidate.dragTargets?.length ? `<p class="hint"><strong>Drop zones:</strong> ${escapeHtml(candidate.dragTargets.join(" · "))}</p>` : ""}
              ${candidate.gridRows?.length ? `<p class="hint"><strong>Table rows:</strong> ${escapeHtml(candidate.gridRows.join(" · "))}</p>` : ""}
              ${mapping || `<p class="hint"><strong>Correct answer:</strong> ${escapeHtml(candidate.answer || "")}</p>`}
              ${candidate.explanation ? `<p class="hint"><strong>AI rationale:</strong> ${escapeHtml(candidate.explanation)}</p>` : ""}
              <div class="bank-selection-actions"><button class="primary" onclick="saveAiFormatDraft(${index})" type="button">Save as Review Draft</button><button class="ghost" onclick="discardAiFormatDraft(${index})" type="button">Discard</button></div>
            </article>
          `;
        })
        .join("")}
...
function renderBankQuestionDetailPanel(question) {
  return `
    <section class="question-detail-panel">
      ...
      <div class="grid two">
        ...
        <div class="stack">
          <div class="grid two compact-grid">
            ...
            <div class="field"><label>Correct answer${["drag_drop", "table_grid"].includes(normalizeQuestionType(question.type)) ? " mapping" : ""}</label><input data-bank-q="${question.id}" data-field="answer" value="${escapeHtml(question.answer || "")}" placeholder="${normalizeQuestionType(question.type) === "drag_drop" ? "One=Odd; Two=Even" : normalizeQuestionType(question.type) === "table_grid" ? "1=Incorrect; 2=Correct" : "Answer"}" /></div>
            <div class="field"><label>${["dropdown", "drag_drop", "table_grid"].includes(normalizeQuestionType(question.type)) ? normalizeQuestionType(question.type) === "table_grid" ? "Table columns" : "Options / drag items" : "Choices"}</label><input data-bank-q="${question.id}" data-field="choices" value="${escapeHtml((question.choices || []).join(", "))}" placeholder="Separate items with commas" /></div>
      ...
      <div class="notice compact"><strong>Format Assistant:</strong> change the answer type, then use the live preview and save. To protect accuracy, the system does not rewrite a multiple-choice question into a new question automatically; you review the prompt, options, and correct answer before saving.</div>
    </section>
  `;
}
  ...
  return `
    <section class="class-linked-strip ${linkedExams.length ? "" : "empty"}">
      ...
      ${
        subtab === "settings"
          ? ""
          : `<button class="ghost" onclick="setAdminSubTab('classes', 'settings')" type="button">Manage Linked Exams</button>`
      }
    </section>
  `;
}
    ...
    <section class="panel stack tool-panel storage-cleanup-panel">
      <div class="section-head">
        <div>
          ...
          <p class="subtle">Delete old completed reports after downloading a backup. Student profiles, exams, and question-bank questions stay untouched.</p>
        </div>
      ...
      <div class="notice compact"><strong>Safe order:</strong> download a Full Backup first. Then choose a date. The cleanup removes reports submitted before that date and their linked completed attempts, which is the data that uses the most database space.</div>
...
async function deleteOldCompletedReports() {
  ...
  try {
    ...
    renderAdmin();
    alert(`${result.deletedReports || 0} old report(s) and ${result.deletedAttempts || 0} linked attempt(s) were deleted.`);
  } catch (error) {
...
function renderExamList(selected, options = {}) {
  ...
  return `
    ...
    ${selected && showSettings ? renderExamSettings(selected) : ""}
    ${selected && showQuestions ? renderQuestionEditor(selected) : ""}
  `;
}
...
function renderExamSettings(exam) {
  return `
    <section class="card stack exam-settings-panel">
      ...
      <label class="row"><input id="editShsatOriginal" type="checkbox" ${exam.scoringMode === "shsat_original" ? "checked" : ""} /> Use Original SHSAT raw-score conversion (exactly 50 questions)</label>
      ${exam.scoringMode === "shsat_original" ? `<div class="notice compact ok"><strong>Original SHSAT chart active.</strong> Reports keep the raw number correct and add the converted SHSAT score from the Original column only.</div>` : ""}
        ...
        <div class="stack">
          <div class="grid two compact-grid">
            ...
            <div class="field">
              ...
              <input data-q="${question.id}" data-field="answer" value="${escapeHtml(question.answer)}" placeholder="${normalizeQuestionType(question.type) === "drag_drop" ? "One=Odd; Two=Even" : normalizeQuestionType(question.type) === "table_grid" ? "1=Incorrect; 2=Correct" : "A, 17, 3/4, √5, 2²"}" />
            </div>
            <div class="field">
              <label>Answer type</label>
            ...
            <div class="field">
              <label>${["dropdown", "drag_drop", "table_grid"].includes(normalizeQuestionType(question.type)) ? normalizeQuestionType(question.type) === "table_grid" ? "Table columns" : "Options / drag items" : "Choices"}</label>
              <input data-q="${question.id}" data-field="choices" value="${escapeHtml((question.choices || []).join(", "))}" placeholder="Separate items with commas" />
            </div>
...
function getQuestionDraftFromDom(questionId, sourceQuestion = {}) {
  ...
  document.querySelectorAll(`[data-q="${questionId}"][data-field]`).forEach((input) => {
    ...
    if (field === "choices") draft.choices = ["dropdown", "drag_drop", "table_grid"].includes(normalizeQuestionType(draft.type)) ? parseOptionValuesInput(value, draft.choices) : parseChoicesInput(value, draft.choices);
    ...
    else if (field === "answer") draft.answer = ["multiple", "numeric"].includes(normalizeQuestionType(draft.type)) ? value.toUpperCase() : value;
...
function renderQuestionLivePreview(question) {
  ...
  const usesInlineBlank = normalizeQuestionType(draft.type) === "dropdown" && String(draft.questionText || "").includes("[[blank]]");
  return `
    ...
    <article class="question preview-question" data-section="${escapeHtml(draft.section)}" data-original-number="${draft.originalNumber || draft.number}">
      ...
      ${usesInlineBlank ? renderDropdownPrompt(draft, "", true) : renderQuestionTextWithInlineMedia(draft, { preview: true, mediaPrefix: "preview-question" })}
      ${draft.imageUrl ? renderEditorImagePreview(draft.imageUrl, "Question image preview") : ""}
      ${usesInlineBlank ? "" : renderStudentAnswerControl(draft, "", { preview: true })}
    </article>
  `;
}
...
function renderDropdownPrompt(question, current = "", preview = false) {
  const text = formatRichText(question.questionText || "");
...
function renderDragDropControl(question, current = "", preview = false) {
  ...
  return `
    <div class="drag-drop-widget" data-question-id="${escapeHtml(question.id)}">
      <div class="drag-drop-head"><p class="hint">Drag an item to a category, or select an item and then select its category.</p><span class="pill ${placedCount === items.length ? "ok" : ""}">Placed ${placedCount} of ${items.length}</span></div>
...
function renderTableGridControl(question, current = "", preview = false) {
  ...
  return `
    <div class="table-grid-widget">
      <p class="hint">For each statement, choose one answer. ${answeredCount} of ${rows.length} completed.</p>
      <div class="table-grid-scroll"><table class="claim-evidence-table"><thead><tr><th scope="col">Statement</th>${columns.map((column) => `<th scope="col">${escapeHtml(column)}</th>`).join("")}</tr></thead><tbody>
...
function renderStudentAnswerControl(question, current = "", options = {}) {
  ...
  if (type === "numeric") return preview ? `<input placeholder="Student short answer: 3/4, √5, 2²" disabled />` : renderShortAnswerInput(question, current);
  if (type === "fill_blank") return `<div class="fill-blank-box"><input class="short-answer-input" value="${escapeHtml(current)}" placeholder="Type your answer" ${preview ? "disabled" : `oninput="setTextAnswer('${escapeJs(question.id)}', this.value)"`} /></div>`;
  ...
  return `<div class="choices">${(question.choices || LETTERS).map((choice) => `<button class="choice ${current === choice ? "selected" : ""}" type="button" ${preview ? "disabled" : `data-original-number="${question.originalNumber || question.number}" data-section="${escapeHtml(question.section)}" data-choice="${escapeHtml(choice)}" onclick="setAnswer('${escapeJs(question.id)}', this.dataset.choice)"`}>${escapeHtml(choice)}</button>`).join("")}</div>`;
}
...
function syncQuestionEditsFromDom(exam) {
  ...
  document.querySelectorAll("[data-q][data-field]").forEach((input) => {
    ...
    const field = input.dataset.field;
    const value = input.value.trim();
    if (field === "choices") question.choices = ["dropdown", "drag_drop", "table_grid"].includes(normalizeQuestionType(question.type)) ? parseOptionValuesInput(value, question.choices) : parseChoicesInput(value, question.choices);
    ...
    else if (field === "answer") question.answer = ["multiple", "numeric"].includes(normalizeQuestionType(question.type)) ? value.toUpperCase() : value;
...
function printSelectedReport() {
  ...
  $("#app").innerHTML = `
    <main class="report-only stack">
      ...
      ${renderSubmissionReport(submission, true, { showProctoring: false, branded: true, compactPrint: true })}
    </main>
  `;
}
...
function getBankQuestionExportStatus(question) {
  ...
  if (["multiple", "dropdown", "drag_drop", "table_grid"].includes(type) && (!Array.isArray(question.choices) || !question.choices.length)) issues.push("missing choices/options");
...
function getShsatOriginalConversion(score, exam) {
  ...
  if (exam?.scoringMode !== "shsat_original") return null;
...
function renderSubmissionReport(submission, showCorrect = true, options = {}) {
  const { showProctoring = true, branded = false, compactPrint = false } = options;
  const exam = state.exams.find((item) => item.id === submission.examId);
  ...
  const reviewItems = missedRows
    .slice(0, 12)
    .map((row) => `${row.section || "Section"} #${row.originalNumber || row.number}`)
    .join(" · ");
  if (compactPrint) {
    return `
      <article class="card stack report-card compact-student-summary ${branded ? "branded-report" : ""}">
        ...
        <div class="compact-report-stats">
          <div><span>Correct</span><strong>${escapeHtml(`${submission.score?.earned ?? 0}/${submission.score?.possible ?? rows.length}`)}</strong></div>
          <div><span>To review</span><strong>${missedRows.length}</strong></div>
          <div><span>Result</span><strong>${escapeHtml(getDiagnosticRating(submission.score?.percent ?? 0))}</strong></div>
        </div>
...
function renderQuestionCsvPreview(summary) {
  ...
  const issueCount = (summary.missingAnswer || 0) + (summary.missingSkill || 0) + (summary.missingDifficulty || 0) + (summary.missingQuestion || 0) + (summary.visualTextOnly || 0) + (summary.errors?.length || 0);
...
function openAiFormatSourceSelection() {
  adminSubTabs.bank = "edit";
  renderAdmin();
}

function getRequestedAiFormatTypes() {
  return [
    ["aiFormatFillBlank", "fill_blank"],
    ["aiFormatDropdown", "dropdown"],
    ["aiFormatDragDrop", "drag_drop"],
    ["aiFormatTableGrid", "table_grid"],
  ]
    .filter(([id]) => Boolean($(`#${id}`)?.checked))
    .map(([, type]) => type);
}

async function generateAiFormatDrafts(event) {
  const available = new Set((state.questionBank || []).map((question) => question.id));
  const questionIds = getCheckedBankQuestionIds().filter((id) => available.has(id));
  const sourceQuestions = (state.questionBank || []).filter((question) => questionIds.includes(question.id));
  const mcqIds = sourceQuestions.filter((question) => normalizeQuestionType(question.type) === "multiple").map((question) => question.id);
  const types = getRequestedAiFormatTypes();
  if (!mcqIds.length) {
    alert("Open Edit, then check one or more original multiple-choice questions first.");
    return;
  }
  if (mcqIds.length > 12) {
    alert("For accuracy and cost control, choose up to 12 MCQs at a time.");
    return;
  }
  if (!types.length) {
    alert("Choose at least one new question format.");
    return;
  }
  const button = event?.currentTarget;
  if (button) button.disabled = true;
  try {
    const result = await api("/api/admin/question-bank/ai-format-drafts", {
      method: "POST",
      body: JSON.stringify({ questionIds: mcqIds, types }),
    });
    formatAssistantDrafts = Array.isArray(result.candidates) ? result.candidates : [];
    if (!formatAssistantDrafts.length) alert("No safe draft was returned for those formats. Try fewer formats or choose another MCQ.");
    renderAdminPreserveScroll();
  } catch (error) {
    alert(error.message);
  } finally {
...
function discardAiFormatDraft(index) {
  formatAssistantDrafts.splice(index, 1);
  renderAdminPreserveScroll();
}

async function saveAiFormatDraft(index) {
  const draft = formatAssistantDrafts[index];
  if (!draft) return;
  if (!confirm("Save this reviewed AI draft to the Question Bank? The original MCQ will remain unchanged.")) return;
  try {
    const result = await api("/api/admin/question-bank/ai-format-drafts/save", {
      method: "POST",
      body: JSON.stringify({ sourceQuestionId: draft.sourceQuestionId, candidate: draft.candidate }),
    });
    state = result.data;
    selectedBankQuestionId = result.question?.id || selectedBankQuestionId;
    formatAssistantDrafts.splice(index, 1);
    alert("Review draft saved to the Question Bank. It is not in an exam yet.");
    renderAdminPreserveScroll();
  } catch (error) {
    alert(error.message);
  }
...
async function createExamFromBank() {
  ...
  const payload = {
    ...
    shsatOriginalScoring: Boolean($("#bankExamShsatOriginal")?.checked),
  };
  try {
...
async function createExam() {
  try {
    ...
    const payload = {
      ...
      shsatOriginalScoring: Boolean($("#examShsatOriginal")?.checked),
    };
...
async function saveExamSettings(id) {
  const payload = {
    ...
    shsatOriginalScoring: Boolean($("#editShsatOriginal")?.checked),
  };
...
async function deleteSubmittedReport(submissionId) {
  ...
  if (!confirm(`Delete submitted report for "${label}"?\n\nThis removes only this test report and its completed attempt record. The exam, question bank, student profile, and class stay saved.`)) return;
...
function renderStudentAllQuestions(questions) {
  ...
  return questions
    .map((question) => {
      ...
      const groupQuestions = questions.filter((item) => String(item.groupId || "").trim() === groupId);
...
function renderStudentStep(questions, stepNumber) {
  if (questions.length <= 1) {
    ...
    if (!question) return "";
    if (question.groupId && (question.passageText || question.passageTitle || question.sharedImageUrl)) return renderStudentPassageGroup(questions, stepNumber);
    return renderStudentQuestion(question, stepNumber, false, true);
  }
...
function renderStudentQuestion(question, displayNumber, hideSharedMedia = false, showOwnPassage = false) {
  ...
  const usesInlineBlank = normalizeQuestionType(question.type) === "dropdown" && String(question.questionText || "").includes("[[blank]]");
  return `
    <article class="question" data-section="${escapeHtml(question.section)}" data-original-number="${question.originalNumber || question.number}">
      ...
      ${question.passageText && (!question.groupId || showOwnPassage) ? renderPassageTextBlock(question, `passage-${question.id}`) : ""}
      ${usesInlineBlank ? renderDropdownPrompt(question, current) : renderQuestionTextWithInlineMedia(question, { mediaPrefix: "student-question" })}
      ${renderQuestionMedia(question, hideSharedMedia)}
      ${usesInlineBlank ? "" : renderStudentAnswerControl(question, current)}
    </article>
  `;
}
