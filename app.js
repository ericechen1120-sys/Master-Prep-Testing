...
let state = { exams: [], submissions: [], students: [], attempts: [], classes: [], questionBank: [], examUsage: [] };
...
let adminWorkspaceLoading = false;
let adminSectionLoadPromise = null;
let loadedAdminSections = new Set();
  ...
  if (data?.stateChanges && typeof data.stateChanges === "object") {
    Object.entries(data.stateChanges).forEach(([section, change]) => {
      if (Array.isArray(change?.replace)) {
        ...
        if (section === "exams") state.examUsage = change.replace.map(makeExamUsageRecord);
        return;
      }
      ...
      if (section === "questionBank" && loadedAdminSections.has("questionBank")) state.questionBankCount = nextItems.length;
      if (section === "exams") {
        const removedUsageIds = new Set((change?.removeIds || []).map(String));
        let nextUsage = (state.examUsage || []).filter((item) => !removedUsageIds.has(String(item?.id || "")));
        upsertItems.forEach((exam) => {
          if (!Array.isArray(exam?.questions)) return;
          const usage = makeExamUsageRecord(exam);
          const existingIndex = nextUsage.findIndex((item) => String(item?.id || "") === String(exam.id || ""));
          if (existingIndex >= 0) nextUsage[existingIndex] = usage;
          else nextUsage.unshift(usage);
        });
        state.examUsage = nextUsage;
      }
    });
...
async function loadAdminState(initialState = null) {
  state = initialState || (await api("/api/admin/state?mode=dashboard"));
  loadedAdminSections = new Set(Array.isArray(state.loadedSections) ? state.loadedSections : []);
  if (!Object.hasOwn(state, "questionBankCount")) state.questionBankCount = state.questionBank?.length || 0;
...
function getQuestionBankCount() {
  return loadedAdminSections.has("questionBank") ? state.questionBank?.length || 0 : Number(state.questionBankCount || 0);
}

function getExamQuestionCount(exam) {
  return Array.isArray(exam?.questions) ? exam.questions.length : Number(exam?.questionCount || 0);
}

function makeExamUsageRecord(exam) {
  return {
    id: exam?.id,
    title: exam?.title,
    code: exam?.code,
    ...
    questions: (exam?.questions || []).map((question) => ({ sourceGroupId: getBankQuestionSourceGroupId(question) })),
  };
}

function getExamUsageRecords() {
  if (loadedAdminSections.has("exams")) return state.exams || [];
  if (Array.isArray(state.examUsage) && state.examUsage.length) return state.examUsage;
  return (state.exams || []).some((exam) => Array.isArray(exam?.questions)) ? state.exams : [];
}

function requiredAdminSections(tab, subtab = "") {
  if (tab === "bank" && subtab !== "import") return ["examUsage", "questionBank"];
  if (tab === "reports") return ["exams"];
  if (tab === "exams" && subtab === "create") return ["questionBank"];
  if (tab === "exams" && subtab === "questions") return ["exams", "questionBank"];
  return [];
}

async function ensureAdminSections(sectionNames = []) {
  const missing = [...new Set(sectionNames)].filter((name) => !loadedAdminSections.has(name));
  if (!missing.length) return;
  if (adminSectionLoadPromise) await adminSectionLoadPromise;
  const stillMissing = missing.filter((name) => !loadedAdminSections.has(name));
  if (!stillMissing.length) return;
  adminSectionLoadPromise = api(`/api/admin/state/sections?names=${encodeURIComponent(stillMissing.join(","))}`)
    .then((result) => {
      Object.entries(result.sections || {}).forEach(([name, items]) => {
        state[name] = Array.isArray(items) ? items : [];
        loadedAdminSections.add(name);
      });
      if (loadedAdminSections.has("questionBank")) state.questionBankCount = state.questionBank.length;
      bankFilteredCache = null;
    })
    .finally(() => {
      adminSectionLoadPromise = null;
    });
  await adminSectionLoadPromise;
}
  ...
  $("#app").innerHTML = `
    <div class="shell">
      ...
      <main class="main grid">
        ...
        ${isOverview || adminWorkspaceLoading ? "" : renderAdminMiniStats()}
        ${adminWorkspaceLoading ? renderAdminWorkspaceLoading() : ""}
        ${!adminWorkspaceLoading && adminActiveTab === "overview" ? renderOverviewWorkspace() : ""}
        ${!adminWorkspaceLoading && adminActiveTab === "reports" ? renderReportsWorkspace(submission) : ""}
        ${!adminWorkspaceLoading && adminActiveTab === "bank" ? renderQuestionBankWorkspace() : ""}
        ${!adminWorkspaceLoading && adminActiveTab === "students" ? renderStudentsWorkspace() : ""}
        ${!adminWorkspaceLoading && adminActiveTab === "classes" ? renderClassesWorkspace() : ""}
        ${!adminWorkspaceLoading && adminActiveTab === "tools" ? renderToolsWorkspace() : ""}
        ${!adminWorkspaceLoading && adminActiveTab === "exams" ? renderExamWorkspace(exam) : ""}
      </main>
    </div>
  `;
  updateSatCalculator();
...
function renderAdminWorkspaceLoading() {
  return `
    <section class="panel stack admin-workspace-loading" aria-live="polite">
      <span class="login-eyebrow">Loading workspace</span>
      <h2>Opening ${escapeHtml(adminTabTitle())}…</h2>
      <p class="subtle">Loading only the detailed questions needed for this workspace.</p>
    </section>
  `;
}
  ...
  return `
    <nav class="admin-tabs ${extraClass}">
      ...
      <button class="${adminActiveTab === "bank" ? "active" : ""}" onclick="setAdminTab('bank')" type="button">
        Questions
        <span>${getQuestionBankCount()} questions</span>
      </button>
...
function renderAdminMiniStats() {
  ...
  const cachedDuplicateExtras = loadedAdminSections.has("questionBank") ? bankDuplicateExtraCache.get(state.questionBank || []) : null;
  const duplicateExtras = Array.isArray(cachedDuplicateExtras) ? cachedDuplicateExtras.length : null;
  return `
    <section class="admin-mini-stats">
      ...
      <button onclick="openAdminWorkspace('bank', 'cleanup')" type="button"><span>Bank cleanup</span><strong>${duplicateExtras ?? "Check"}</strong></button>
  ...
  return (
    {
      ...
      bank: [
        ...
        { id: "edit", label: "Edit", meta: `${getQuestionBankCount()} questions` },
...
async function switchAdminWorkspace(tab, subtab = "") {
  if (!sessionStorage.getItem(TOKEN_KEY)) {
    adminLogin();
    return;
  }
  const previousTab = adminActiveTab;
  const previousSubtab = getAdminSubTab(previousTab);
  adminActiveTab = tab;
  if (subtab) adminSubTabs[tab] = subtab;
  else adminSubTabs[tab] = getAdminSubTab(tab);
  const requiredSections = requiredAdminSections(tab, adminSubTabs[tab]);
  const missingSections = requiredSections.filter((name) => !loadedAdminSections.has(name));
  ...
  renderAdmin();
  try {
    await ensureAdminSections(missingSections);
    await new Promise((resolve) => requestAnimationFrame(resolve));
  } catch (error) {
    adminActiveTab = previousTab;
    adminSubTabs[previousTab] = previousSubtab;
    alert(error.message || "This workspace could not be loaded. Please try again.");
  } finally {
...
function setAdminSubTab(tab, subtab) {
  return switchAdminWorkspace(tab, subtab);
}

function setAdminTab(tab) {
  return switchAdminWorkspace(tab, getAdminSubTab(tab));
}

function openAdminWorkspace(tab, subtab = "") {
  return switchAdminWorkspace(tab, subtab || getAdminSubTab(tab));
}
...
function renderOverviewWorkspace() {
  const recentReports = [...(state.submissions || [])]
    ...
    .slice(0, 6);
  const bankSummary = loadedAdminSections.has("questionBank")
    ? (state.questionBank || []).reduce((map, question) => {
        const label = getBankQuestionClass(question) || question.examType || "Unassigned";
        map.set(label, (map.get(label) || 0) + 1);
        return map;
      }, new Map())
    : new Map((state.questionBankSummary || []).map((item) => [item.label, item.count]));
  ...
  return `
    <section class="admin-workspace stack">
      <section class="admin-kpis grid six">
        ...
        <button class="stat action-stat" onclick="openAdminWorkspace('bank', 'smart')" type="button"><span class="stat-icon">QB</span><span class="subtle">Question Bank</span><strong>${getQuestionBankCount()}</strong><small>Smart draft from bank</small></button>
...
function renderAdminAttentionQueue() {
  ...
  const emptyExams = (state.exams || []).filter((exam) => getExamQuestionCount(exam) === 0);
  ...
  const duplicateExtras = loadedAdminSections.has("questionBank") ? bankDuplicateExtraCache.get(state.questionBank || []) || [] : [];
      ...
      <section class="admin-workspace stack">
        ...
        <div class="panel stack">
          <div class="section-head">
            <div>
              <h2>Question Bank</h2>
              <p class="subtle">Import new questions without loading the complete bank into this screen.</p>
            </div>
            <span class="pill">${getQuestionBankCount()} saved</span>
          </div>
  ...
  return `
    <div class="stack build-workflow-grid">
      ...
      <div class="card stack question-mix-builder">
        ...
        <div class="mix-optional-formats mix-format-picker">
          <div class="section-head">
            <div><h4>Question Types</h4><p class="hint">Enter exactly how many of each type you want. Leave a type at zero to exclude it.</p></div>
            <button class="ghost small" onclick="resetOptionalQuestionFormats()" type="button">Set Optional Types to Zero</button>
          </div>
    ...
    <section class="card stack">
      <div class="section-head">
        <div>
          ...
          <p class="subtle">Checks all ${getQuestionBankCount()} saved questions without changing or deleting any question.</p>
        </div>
...
function getFilteredBankQuestions(options = {}) {
  ...
  const examUsage = getExamUsageRecords();
  ...
  if (bankFilteredCache?.bank === bank && bankFilteredCache.examUsage === examUsage && bankFilteredCache.key === cacheKey) return bankFilteredCache.questions;
  ...
  bankFilteredCache = { bank, examUsage, key: cacheKey, questions };
  return questions;
}
...
function resetOptionalQuestionFormats() {
  ["mixFillBlank", "mixDropdown", "mixDragDrop", "mixTableGrid", "mixNumeric", "mixEquation", "mixHotText", "mixHotspot"].forEach((id) => {
...
function getUsedBankSourceIds() {
  const used = new Set();
  getExamUsageRecords().forEach((exam) => {
...
function renderProgramDatalist(id) {
  const programs = [...new Set(getExamUsageRecords().map(getExamProgramName).filter(Boolean))].sort((left, right) => left.localeCompare(right));
                  ...
                  (exam) => `
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>${escapeHtml(exam.code || "No code")} · ${escapeHtml(exam.examType || "Exam")} · ${getExamQuestionCount(exam)} questions</small>
                      </span>
                    </label>
                  `
                )
  ...
  return `
    <section class="panel stack tool-panel">
      ...
      <div class="grid three">
        ...
        <div class="stat"><span class="subtle">Reports</span><strong>${state.submissions.length}</strong></div>
        <div class="stat"><span class="subtle">Question Bank</span><strong>${getQuestionBankCount()}</strong></div>
      </div>
      <div class="bank-selection-actions">
...
function renderQuestionEditorExamPicker(selected) {
  ...
  const selectedLabel = selected ? `${selected.examType === "math" ? "Math" : "English"} · ${selected.code || "No code"} · ${getExamQuestionCount(selected)} questions` : "No exam selected";
...
async function addSingleBankQuestion() {
  try {
    const previousBankCount = getQuestionBankCount();
    ...
    if (result.question) {
      ...
      if (!loadedAdminSections.has("questionBank")) state.questionBankCount = previousBankCount + 1;
      bankFilteredCache = null;
    } else {
      state = result;
    }
...
async function getLoadedExam(examId) {
  try {
    await ensureAdminSections(["exams"]);
    return state.exams.find((item) => item.id === examId) || null;
  } catch (error) {
    alert(error.message || "The exam details could not be loaded. Please try again.");
    return null;
  }
}

async function downloadExamBackup(examId) {
  const exam = await getLoadedExam(examId);
  if (!exam) return;
...
async function printTeacherExamVersion(examId) {
  const exam = await getLoadedExam(examId);
  if (!exam) {
...
async function printStudentExamVersion(examId) {
  const exam = await getLoadedExam(examId);
  if (!exam) {
    alert("Select an exam first.");
    return;
...
async function openPaperExamForm(examId) {
  await printStudentExamVersion(examId);
}
...
async function importQuestionBankCsv() {
  ...
  try {
    ...
    if (Array.isArray(result.importedQuestions)) {
      ...
      if (!loadedAdminSections.has("questionBank")) state.questionBankCount = previousBankCount + result.importedQuestions.length;
      bankFilteredCache = null;
    } else {
      state = result;
    }
...
function openBankClass(value) {
  ...
  bankPage = 0;
  bankAiPlan = null;
  return openAdminWorkspace("bank", "build");
}
