...
function renderTopbar(active = "admin", showAdminTabs = false) {
  ...
  `;
  /* Legacy embedded logo kept only for compatibility with older saved copies.
  return `
    <header class="topbar">
      <div class="brand">
    ...
    <div class="bank-filters">
      ...
      <div class="field">
        ...
        <select onchange="setBankSubjectFilter(this.value)">
          ...
          <option value="Reading Comprehension" ${bankSubjectFilter === "Reading Comprehension" ? "selected" : ""}>Reading Comprehension</option>
      ...
      <div class="field">
        <label>Existing exam use</label>
        <select onchange="setBankUsageFilter(this.value)">
          <option value="all" ${bankUsageFilter === "all" ? "selected" : ""}>All questions</option>
          <option value="unused" ${bankUsageFilter === "unused" ? "selected" : ""}>Unused questions only</option>
        </select>
      </div>
    </div>
...
function renderBankBuildPanel(questions) {
  ...
  const mixReadingCount = questionMixPlan?.readingTarget ?? (isElaBuild ? Math.round(mixTarget * 0.8) : 0);
  const mixReadingPercent = questionMixPlan?.readingPercent ?? (mixTarget ? Math.round((mixReadingCount / mixTarget) * 100) : 0);
  const mixTargetStudentId = questionMixPlan?.targetStudentId || ((state.students || []).length === 1 ? state.students[0].id : "");
  return `
    <div class="grid two">
      <div class="card stack">
      ...
      <div class="card stack question-mix-builder">
        <div class="section-head"><div><h3>Flexible Question Mix</h3><p class="subtle">Choose any difficulty levels, an exact reading-comprehension count, and only the answer formats you want.</p></div><span class="pill warn">Review draft</span></div>
        <div class="grid four compact-grid">
          ...
          <div class="field"><label>Reading comprehension % <span class="label-note">ELA default: 80%</span></label><input id="mixReadingPercent" type="number" min="0" max="100" value="${mixReadingPercent}" oninput="updateFlexibleReadingFromPercent()" /></div>
          <div class="field"><label>Reading comprehension <span class="label-note">exact question count</span></label><input id="mixReadingCount" type="number" min="0" value="${mixReadingCount}" oninput="updateFlexibleReadingPercentFromCount()" /></div>
        ...
        <label class="row"><input id="mixAvoidUsed" type="checkbox" ${questionMixPlan?.avoidUsed === false ? "" : "checked"} /> Exclude questions already used in any existing exam</label>
...
function getFilteredBankQuestions(options = {}) {
  ...
  const usedSourceIds = bankUsageFilter === "unused" ? getUsedBankSourceIds() : null;
  const cacheKey = [search, bankClassFilter, bankSubjectFilter, bankSkillFilter, bankDifficultyFilter, bankTypeFilter, bankUsageFilter, ignoreType ? "ignore-type" : "with-type", ignoreDifficulty ? "ignore-difficulty" : "with-difficulty"].join("|");
...
function getQuestionMixRequest() {
  ...
  const readingTarget = Math.max(0, Number.parseInt($("#mixReadingCount")?.value, 10) || 0);
  const difficultyLevels = [1, 2, 3, 4].filter((level) => $("#mixDifficulty" + level)?.checked);
  const avoidUsed = $("#mixAvoidUsed")?.checked !== false;
  const targetStudentId = $("#mixTargetStudentId")?.value || "";
  const readingPercent = target ? Math.round((readingTarget / target) * 100) : 0;
  return { target, requested, optionalTotal, readingTarget, readingPercent, difficultyLevels, avoidUsed, targetStudentId };
}
...
function updateFlexibleReadingPercentFromCount() {
  syncReadingPercentFromCount("#mixTotal", "#mixReadingPercent", "#mixReadingCount");
}

function buildQuestionMixDraft() {
  const { target, requested, optionalTotal, readingTarget, readingPercent, difficultyLevels, avoidUsed, targetStudentId } = getQuestionMixRequest();
  ...
  if (readingTarget > target) configurationErrors.push("Reading comprehension count cannot be larger than the total question count.");
  if (configurationErrors.length) {
    return { target, requested, selected: [], shortfalls: [], configurationErrors, difficultyLevels, readingTarget, readingPercent, readingCount: 0, difficultyCounts: {}, passageSetCount: 0, poolCount: 0, avoidUsed, targetStudentId, complete: false };
  }

  const allowedLevels = new Set(difficultyLevels);
  const usedSourceIds = avoidUsed ? getUsedBankSourceIds() : new Set();
  const studentSeenSourceIds = targetStudentId ? getStudentSeenBankSourceIds(targetStudentId) : new Set();
  const allQuestions = state.questionBank || [];
  const anchorPool = getFilteredBankQuestions({ ignoreType: true, ignoreDifficulty: true });
  const units = new Map();
  anchorPool.forEach((anchor) => {
    ...
    const alreadyUsed = unitQuestions.some((question) => {
      ...
      return (avoidUsed && usedSourceIds.has(sourceId)) || studentSeenSourceIds.has(sourceId);
    });
    if (invalid || wrongDifficulty || alreadyUsed) return;
  ...
  const availableUnits = [...units.values()]
    ...
    .filter((unit) => typeKeys.every((type) => unit.typeCounts[type] <= requested[type]) && unit.readingCount <= readingTarget);
  ...
  availableUnits.forEach((unit, unitIndex) => {
    ...
    mixStates.forEach((mixState) => {
      ...
      if (!fits || nextReadingCount > readingTarget) return;
      const nextState = {
        counts: nextCounts,
  ...
  const exactKey = stateKey(requested, readingTarget);
  let chosenState = mixStates.get(exactKey);
  if (!chosenState) {
  ...
  if (chosenState.readingCount !== readingTarget) shortfalls.push({ type: "Reading comprehension", missing: Math.abs(readingTarget - chosenState.readingCount), message: `The available complete passage sets cannot make exactly ${readingTarget} reading-comprehension question${readingTarget === 1 ? "" : "s"}.` });
  const complete = Boolean(mixStates.get(exactKey));
  return {
    target,
    requested,
    selected,
  ...
  return `
    <div class="mix-draft-summary">
      ...
      <div class="selection-pills">
        ...
        <span class="pill ${plan.readingCount === plan.readingTarget ? "ok" : "warn"}">Reading: ${plan.readingCount}/${plan.readingTarget}</span>
        ...
        ${plan.avoidUsed ? `<span class="pill ok">Unused questions only</span>` : `<span class="pill warn">Previously used allowed</span>`}
      ...
      ${plan.shortfalls?.length ? `<div class="notice bad">${plan.shortfalls.map((item) => escapeHtml(item.message)).join(" ")}</div>` : plan.complete ? `<div class="notice ok">Exact mix is ready. Review or check the draft before creating the exam.</div>` : ""}
    </div>
  `;
}
...
function buildSmartBankExamDraft() {
  ...
  if (configurationErrors.length) {
    return { createdAt: new Date().toISOString(), requested: targetCount, selected, rejected, poolCount: 0, passageSetCount: 0, mode, options, avoidUsed, targetStudentId, hasExactDifficultyMix, difficultyTargets, readingTarget, readingCount: 0, difficultyCounts: {}, configurationErrors, complete: false };
  }
  const allQuestions = state.questionBank || [];
...
function renderSmartExamGenerator(questions) {
  ...
  const smartTargetStudentId = bankAiPlan?.targetStudentId || ((state.students || []).length === 1 ? state.students[0].id : "");
  return `
    <section class="card stack bank-ai-card">
      <div class="section-head">
        <div>
...
function selectBankQuestion(id) {
  selectedBankQuestionId = id;
  const index = getBankEditorQuestions(getFilteredBankQuestions()).findIndex((question) => question.id === id);
...
function setBankListSort(value) {
  ...
  bankPage = 0;
  renderAdminPreserveScroll("#bankListSort");
}
...
async function createExamFromSelectedBankQuestions(explicitQuestionIds = null, mixConstraints = null, targetStudentId = "") {
  const availableIds = new Set((state.questionBank || []).map((question) => question.id));
