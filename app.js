...
function questionChoicesLabel(type) {
  ...
  if (normalized === "table_grid") return "Table columns";
...
function questionAnswerPlaceholder(type) {
  ...
  if (normalized === "table_grid") return "1=Incorrect; 2=Correct";
...
function buildSmartBankExamDraft() {
  ...
  sequence.forEach((targetDifficulty) => {
    ...
    selectedSourceGroups.add(getBankQuestionSourceGroupId(chosen));
  ...
  if (selected.length < targetCount) {
    pool.forEach((question) => {
      ...
      if (selectedSourceGroups.has(sourceGroupId)) {
        rejected.push(question);
        return;
      }
      selected.push(question);
      duplicateSignatures.add(signature);
      selectedSourceGroups.add(sourceGroupId);
    });
  }
  ...
  return `
    <section class="question-detail-panel">
      ...
      <div class="field">
        ...
        <p class="hint">Versions of the same original question must share this ID. Auto-created formats are linked automatically, and only one version can be used in an exam.</p>
      </div>
...
function renderHotTextPrompt(question, current = "", preview = false) {
  ...
  let text = formatRichText(question.questionText || "");
  let matched = false;
...
function renderStudentAnswerControl(question, current = "", options = {}) {
  ...
  if (["numeric", "equation"].includes(type)) return preview ? `<input placeholder="Student math answer: 3/4, √5, 2², x=4" disabled />` : renderShortAnswerInput(question, current, type === "equation");
...
async function createExamFromBank() {
  ...
  try {
    ...
    alert(`Exam created with ${createdExam.questions.length} bank question${createdExam.questions.length === 1 ? "" : "s"}.${omitted ? ` ${omitted} alternate version${omitted === 1 ? " was" : "s were"} left out so the same source question cannot appear twice.` : ""}`);
    renderAdmin();
  } catch (error) {
    alert(error.message);
  }
...
function findCreatedBankExam(exams, bankQuestionIds, title, code) {
  ...
  return [...(exams || [])]
    ...
    .find((exam) => {
      ...
      return hasMatchingQuestions && (exam.questions || []).length === selectedSourceGroups.size;
    });
}
...
function renderStudentQuestion(question, displayNumber, hideSharedMedia = false, showOwnPassage = false) {
  ...
  return `
    <article class="question" data-section="${escapeHtml(question.section)}" data-original-number="${question.originalNumber || question.number}">
      ...
      ${usesInlineBlank || usesHotText ? "" : renderStudentAnswerControl(question, current)}
    </article>
  `;
}

function renderShortAnswerInput(question, current, equationMode = false) {
  const keypadButtons = equationMode ? [
    ...
    { label: "a/b", insert: "/", offset: 1 }, { label: "π", insert: "π", offset: 1 }, { label: "(", insert: "(", offset: 1 }, { label: ")", insert: ")", offset: 1 }, { label: ".", insert: ".", offset: 1 },
  ] : [
