              ...
              .map(
                (question) => `
                  <tr>
                    ...
                    <td><textarea data-q="${question.id}" data-field="questionText" placeholder="Optional text">${escapeHtml(question.questionText || "")}</textarea></td>
                    <td>
                      <input data-q="${question.id}" data-field="imageUrl" value="${escapeHtml(question.imageUrl || "")}" placeholder="Image URL or upload" />
                      <div class="row tight">
                        <input type="file" accept="image/*" onchange="uploadQuestionImage('${question.id}', 'imageUrl', this.files[0])" />
                        <button class="ghost mini" onclick="setQuestionImage('${question.id}', 'math')" type="button">Sample</button>
                      </div>
                    </td>
                    <td>
                      <input data-q="${question.id}" data-field="sharedImageUrl" value="${escapeHtml(question.sharedImageUrl || "")}" placeholder="Passage image" />
                      <div class="row tight">
                        <input type="file" accept="image/*" onchange="uploadQuestionImage('${question.id}', 'sharedImageUrl', this.files[0])" />
                        <button class="ghost mini" onclick="setQuestionImage('${question.id}', 'passage')" type="button">Sample</button>
                      </div>
                    </td>
                  </tr>
                `
...
function renderSubmissionReport(submission, showCorrect = true) {
  const exam = state.exams.find((item) => item.id === submission.examId);
  const sat = showCorrect ? submission.score.satEstimate : null;
      ...
      ${
        showCorrect && sat
          ? `<div class="grid three">
              ...
              <div class="stat"><span class="subtle">SAT Estimate</span><strong>${sat.totalScore || "N/A"}</strong><span class="subtle">Calculator</span></div>
            </div>`
          : ""
      }
      ${
