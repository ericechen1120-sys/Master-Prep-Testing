    ...
    <div class="grid two">
      ...
      <div class="card stack question-mix-builder">
        ...
        <div class="grid four compact-grid">
          <div class="field"><label>Total questions</label><input id="mixTotal" type="number" min="1" value="${questionMixPlan?.target || 50}" oninput="updateFlexibleMixTotals()" /></div>
          ...
          <div class="field"><label>Reading comprehension <span class="label-note">exact question count</span></label><input id="mixReadingCount" type="number" min="0" value="${questionMixPlan?.readingTarget ?? 0}" /></div>
...
function getFilteredBankQuestions(options = {}) {
  ...
  const cacheKey = [search, bankClassFilter, bankSubjectFilter, bankSkillFilter, bankDifficultyFilter, bankTypeFilter, ignoreType ? "ignore-type" : "with-type", ignoreDifficulty ? "ignore-difficulty" : "with-difficulty"].join("|");
...
function getQuestionMixRequest() {
  ...
  return { target, requested, optionalTotal, readingTarget, difficultyLevels };
}
...
function buildQuestionMixDraft() {
  const { target, requested, optionalTotal, readingTarget, difficultyLevels } = getQuestionMixRequest();
  ...
  if (configurationErrors.length) {
    return { target, requested, selected: [], shortfalls: [], configurationErrors, difficultyLevels, readingTarget, readingCount: 0, difficultyCounts: {}, passageSetCount: 0, poolCount: 0, complete: false };
  }
  ...
  anchorPool.forEach((anchor) => {
    ...
    if (invalid || wrongDifficulty) return;
...
async function createExamFromSelectedBankQuestions(explicitQuestionIds = null, mixConstraints = null) {
  const availableIds = new Set((state.questionBank || []).map((question) => question.id));
