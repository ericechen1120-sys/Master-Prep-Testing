    ...
    <section class="grid two">
      <div class="panel stack">
        ...
        <div class="csv-import-box">
          ...
          <div class="field">
            <label>Default Test / Class</label>
            <input id="bankImportTestClass" placeholder="Example: SHSAT Math, SAT English, Spring 26" />
            <p class="hint">This label will be added to every question in this CSV. You can edit it later.</p>
          </div>
        ...
        <div class="card stack">
          ...
          <div class="bank-selection-actions">
            ...
            <button class="ghost" onclick="createExamFromSelectedBankQuestions()" type="button">Create Exam From Checked (${selectedBankQuestionIds.size})</button>
          </div>
        ...
        <div class="card stack">
          ...
          <div class="bank-selection-actions">
            ...
            <button class="ghost" onclick="printTeacherBankVersion(true)" type="button">Teacher Version From Checked (${selectedBankQuestionIds.size})</button>
          </div>
...
function getFilteredBankQuestions() {
  ...
  return [...(state.questionBank || [])]
    ...
    .sort((left, right) => (Number(left.bankNumber) || 999999) - (Number(right.bankNumber) || 999999) || (Number(left.originalNumber) || 999999) - (Number(right.originalNumber) || 999999));
}
    ...
    <div class="question-editor-layout bank-editor-layout">
      <aside class="question-nav-list">
        <div class="bank-picker-toolbar">
          <span class="pill">${selectedBankQuestionIds.size} checked</span>
    ...
    <section class="question-detail-panel">
      ...
      <div class="grid two">
        <div class="field">
          ...
          <textarea data-bank-q="${question.id}" data-field="questionText">${escapeHtml(question.questionText || "")}</textarea>
        </div>
        <div class="stack">
          <div class="grid two compact-grid">
      ...
      <div class="grid two">
        ...
        <div class="field"><label>Passage text</label><textarea data-bank-q="${question.id}" data-field="passageText">${escapeHtml(question.passageText || "")}</textarea></div>
      </div>
      <div class="grid two">
        <div class="field"><label>Question image URL</label><input data-bank-q="${question.id}" data-field="imageUrl" value="${escapeHtml(question.imageUrl || "")}" /></div>
        <div class="field"><label>Shared passage image URL</label><input data-bank-q="${question.id}" data-field="sharedImageUrl" value="${escapeHtml(question.sharedImageUrl || "")}" /></div>
      </div>
      <div class="field">
    ...
    <section class="question-detail-panel">
      ...
      <div class="grid two">
        ...
        <div class="field">
          ...
          <textarea data-q="${question.id}" data-field="passageText" placeholder="Shared passage text for this group">${escapeHtml(question.passageText || "")}</textarea>
        </div>
      </div>
      <div class="grid two">
        <div class="field">
...
function printTeacherBankVersion(checkedOnly = false) {
  ...
  $("#app").innerHTML = `
    <main class="report-only stack teacher-version">
      ...
      ${questions.map((question, index) => renderTeacherBankQuestion(question, index)).join("")}
    </main>
  `;
}
...
function printTeacherExamVersion(examId) {
  ...
  $("#app").innerHTML = `
    <main class="report-only stack teacher-version">
      ...
      ${questions.map((question, index) => renderTeacherExamQuestion(question, index, exam)).join("")}
    </main>
  `;
}
...
function renderTeacherBankQuestion(question, index) {
  ...
  return `
    <article class="card stack teacher-question">
      <div class="section-head">
        <div>
          <h3>Question ${index + 1}</h3>
          <p class="subtle">${escapeHtml(question.testClass || "No class")} · ${escapeHtml(question.sourceQuestionId || "No ID")} · ${escapeHtml(question.section || "Section")} #${escapeHtml(question.originalNumber || question.number || index + 1)}</p>
        </div>
        <span class="pill">${question.difficulty ? `Level ${escapeHtml(question.difficulty)}` : "No level"}</span>
      </div>
      ${renderTeacherPassageBlock(question)}
      <div class="teacher-question-text">${formatQuestionText(question.questionText || "No question text saved.")}</div>
    ...
    <article class="card stack teacher-question">
      <div class="section-head">
        <div>
          ...
          <p class="subtle">${escapeHtml(exam.examType === "math" ? "Math" : "English")} · ${escapeHtml(question.sourceQuestionId || "No bank ID")} · ${escapeHtml(question.section || "Section")} #${escapeHtml(question.originalNumber || question.number || index + 1)}</p>
        </div>
        <span class="pill">${question.difficulty ? `Level ${escapeHtml(question.difficulty)}` : "No level"}</span>
      </div>
      ${renderTeacherPassageBlock(question)}
...
function renderTeacherPassageBlock(question) {
  ...
  return `
    <div class="teacher-passage">
      ...
      <p>${formatQuestionText(question.passageText)}</p>
    </div>
  `;
}
...
function analyzeQuestionCsv(text) {
  ...
  const summary = {
    ...
    examples: [],
    rowIssues: [],
    errors: missingHeaders.length ? [`Missing required column: ${missingHeaders.join(", ")}`] : [],
  };

  rows.slice(1).forEach((row, index) => {
    const question = cleanCsvValue(getCsvValue(row, headers, ["Question", "Prompt", "Question Text"]));
    const correctLetter = cleanCsvValue(getCsvValue(row, headers, ["Correct Answer Letter", "Answer Letter", "Answer"]));
    const correctText = cleanCsvValue(getCsvValue(row, headers, ["Correct Answer Text", "Answer Text", "Grid-In Answer"]));
    const type = cleanCsvValue(getCsvValue(row, headers, ["Question Type", "Type"])).toLowerCase();
    const skill = cleanCsvValue(getCsvValue(row, headers, ["Skill", "Domain", "Standard"]));
    const difficulty = cleanCsvValue(getCsvValue(row, headers, ["Difficulty (1-4)", "Difficulty", "Level"]));
    const groupId = cleanCsvValue(getCsvValue(row, headers, ["Passage ID", "Group", "Group ID"]));
    const passageTitle = cleanCsvValue(getCsvValue(row, headers, ["Passage Title", "Stimulus Title", "Title"]));
    const passageText = cleanCsvValue(getCsvValue(row, headers, ["Passage / Stimulus", "Passage", "Stimulus", "Reading Passage"]));
  ...
  try {
    state = await api("/api/admin/question-bank/import", {
      ...
      body: JSON.stringify({
        ...
        testClass: $("#bankImportTestClass")?.value || "",
      }),
    });
...
function renderStudentStep(questions, stepNumber) {
  ...
  return `
    <section class="question-group">
      ...
      ${renderPassageTextBlock(passageQuestion)}
...
function renderPassageTextBlock(question) {
  if (!question?.passageText) return "";
  return `
    <section class="passage-text-block">
      ${question.passageTitle ? `<h3>${escapeHtml(question.passageTitle)}</h3>` : ""}
      <div>${formatQuestionText(question.passageText)}</div>
    </section>
  `;
}
...
function renderStudentQuestion(question, displayNumber, hideSharedMedia = false) {
  ...
  return `
    <article class="question" data-section="${escapeHtml(question.section)}" data-original-number="${question.originalNumber || question.number}">
      ...
      ${question.passageText && !question.groupId ? renderPassageTextBlock(question) : ""}
