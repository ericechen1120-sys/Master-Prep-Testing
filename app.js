  ...
  $("#app").innerHTML = `
    <div class="student-exam-shell">
      <header class="student-exam-topbar">
        <div class="student-exam-brand" aria-label="Topway Prep"><img class="student-exam-logo" src="assets/topway-prep-logo-transparent.png" alt="Topway Prep" /></div>
  ...
  return `
    <section class="card stack">
      ...
      ${inspection ? `
        ...
        ${inspection.invalid.length ? `<div class="notice warn compact"><strong>Nothing was changed.</strong> Open each flagged question to review its highlighted issue before using it in a new exam.</div><div class="table-wrap"><table><thead><tr><th>Question ID</th><th>Type</th><th>Issue</th><th></th></tr></thead><tbody>${inspection.invalid.slice(0, 40).map((item) => `<tr><td>${escapeHtml(item.question.sourceQuestionId || item.question.id)}</td><td>${escapeHtml(questionTypeLabel(item.question.type))}</td><td>${escapeHtml(item.issues.join(", "))}</td><td><button class="ghost" onclick="openInspectedBankQuestion('${item.question.id}')" type="button">Open</button></td></tr>`).join("")}</tbody></table></div>${inspection.invalid.length > 40 ? `<p class="hint">Showing the first 40 flagged questions. Use the filters and inspection again after correcting them.</p>` : ""}` : `<div class="notice ok">All checked questions are ready for use.</div>`}
...
function renderStudentAnswerControl(question, current = "", options = {}) {
  ...
  if (type === "fill_blank") return isMathQuestion(question) ? (preview ? `<input placeholder="Student math answer: 3/4, √5, ∞" disabled />` : renderShortAnswerInput(question, current, false)) : `<div class="fill-blank-box"><input class="short-answer-input" value="${escapeHtml(current)}" placeholder="Type your answer" ${preview ? "disabled" : `oninput="setTextAnswer('${escapeJs(question.id)}', this.value)"`} /></div>`;
...
async function saveAiFormatDraft(index) {
  ...
  try {
    ...
    state = result.question ? { ...state, questionBank: [result.question, ...(state.questionBank || [])] } : result.data || result;
...
async function createExamFromBank() {
  ...
  try {
    ...
    alert(`Exam created with ${createdExam.questions.length} bank question${createdExam.questions.length === 1 ? "" : "s"}.${omitted ? ` ${omitted} alternate version${omitted === 1 ? " was" : "s were"} left out so the same source question cannot appear twice.` : ""}${needsReview ? ` ${needsReview} incomplete question${needsReview === 1 ? " was" : "s were"} left out for review.` : ""}`);
    renderAdmin();
  } catch (error) {
    alert(error.message);
  }
    ...
    <div class="student-exam-shell">
      <header class="student-exam-topbar">
        <div class="student-exam-brand" aria-label="Topway Prep">
          <img class="student-exam-logo" src="assets/topway-prep-logo-transparent.png" alt="Topway Prep" />
        </div>
        ...
        ${
          hasPassageContent
            ? `<aside class="passage-column student-passage-pane">
                <div class="student-pane-label">Passage</div>
