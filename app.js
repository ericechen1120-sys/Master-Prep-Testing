...
function renderSmartExamDraft(plan) {
  ...
  return `
    <div class="smart-draft" data-smart-bank-draft>
      ...
      ${!plan.configurationErrors?.length && !plan.complete ? `<div class="notice warn"><strong>Exact mix not available.</strong> No exam will be created from this draft. Adjust the reading count, Level 1–4 counts, source filter, or complete-passage limit and generate again.</div>` : ""}
      <div class="csv-health">
...
function renderSmartExamGenerator(questions) {
  ...
  return `
    <section class="card stack bank-ai-card">
      ...
      <div class="grid four compact-grid">
        ${[1, 2, 3, 4].map((level) => `<div class="field"><label>Level ${level} questions</label><input id="aiExamDifficulty${level}" type="number" min="0" placeholder="Auto" value="${bankAiPlan?.hasExactDifficultyMix ? bankAiPlan.difficultyTargets?.[level] || 0 : ""}" /></div>`).join("")}
      </div>
      <p class="hint">Leave all Level boxes blank for the selected difficulty style. Enter Level counts for an exact custom mix; their total becomes the exam question count. Reading questions keep their complete passages.</p>
...
function useSmartBankDraft() {
  ...
  if (!bankAiPlan.complete) {
    alert("This draft does not match the exact reading and difficulty counts. Adjust the mix and generate again.");
    return;
  }
...
async function createExamFromSmartBankDraft() {
  ...
  if (!bankAiPlan.complete) {
    alert("This draft does not match the exact reading and difficulty counts, so no exam was created.");
    return;
  }
  selectedBankQuestionIds = new Set(bankAiPlan.selected.map((question) => question.id));
  await createExamFromSelectedBankQuestions();
