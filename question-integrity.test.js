...
const incompleteStrictPassage = resolveStrictMixSelection([bankQuestion, secondBankQuestion], [bankQuestion]);
assert.deepEqual(incompleteStrictPassage.missingCompanions.map((question) => question.sourceQuestionId), ["BANK-2"]);
const completeStrictPassage = resolveStrictMixSelection([bankQuestion, secondBankQuestion], [bankQuestion, secondBankQuestion]);
assert.equal(completeStrictPassage.missingCompanions.length, 0);

const standaloneOne = readingQuestion({ id: "standalone-1", sourceQuestionId: "STANDALONE-1", sourceGroupId: "STANDALONE-1", groupId: "", passageTitle: "", passageText: "", number: 1 });
const passageOne = { ...bankQuestion, id: "passage-1", number: 2 };
const passageTwo = { ...secondBankQuestion, id: "passage-2", number: 3 };
const standaloneTwo = readingQuestion({ id: "standalone-2", sourceQuestionId: "STANDALONE-2", sourceGroupId: "STANDALONE-2", groupId: "", passageTitle: "", passageText: "", number: 4 });
const sourceOrderExam = { shuffle: false, questions: [standaloneOne, passageOne, passageTwo, standaloneTwo] };
assert.deepEqual(buildDisplayOrder(sourceOrderExam, () => 0), ["standalone-1", "passage-1", "passage-2", "standalone-2"]);
const randomizedOrder = buildDisplayOrder({ ...sourceOrderExam, shuffle: true }, () => 0);
assert.notDeepEqual(randomizedOrder, ["standalone-1", "passage-1", "passage-2", "standalone-2"]);
assert.equal(randomizedOrder.indexOf("passage-2"), randomizedOrder.indexOf("passage-1") + 1);

const exactMixConstraints = {
  target: 1,
  typeCounts: { multiple: 1, hot_text: 0, table_grid: 0, fill_blank: 0 },
  readingTarget: 1,
  difficultyLevels: [],
};
assert.equal(validateExamMixConstraints([bankQuestion], exactMixConstraints), "");
const unwantedHotText = { ...bankQuestion, type: "hot_text" };
assert.match(validateExamMixConstraints([unwantedHotText], exactMixConstraints), /answer formats.*hot text: 1\/0/i);
...
console.log("Question integrity checks passed: saved exam order, passage-safe student randomization, passage repair, strict CSV validation, duplicate prevention, passage identity, strict Flexible Mix validation, and safe Question-ID loading.");
