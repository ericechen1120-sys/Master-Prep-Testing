...
state.exams[0].program = 'Summer SHSAT';
assert.equal(getReusePolicyBlockedSourceIds('different_program', 'Summer SHSAT').has('grammar-1'), true);
assert.equal(getReusePolicyBlockedSourceIds('different_program', 'Fall SHSAT').has('grammar-1'), false);
assert.equal(getReusePolicyBlockedSourceIds('allow_previous', 'Summer SHSAT').size, 0);
fields['#bankExamReusePolicy'] = { value: 'different_program' };
fields['#bankExamProgram'] = { value: '' };
const missingProgramPlan = buildQuestionMixDraft();
assert.equal(missingProgramPlan.complete, false);
assert.ok(missingProgramPlan.configurationErrors.some((message) => message.includes('Program / Course')));
fields['#bankExamProgram'].value = 'Fall SHSAT';
const crossProgramPlan = buildQuestionMixDraft();
assert.equal(crossProgramPlan.reusePolicy, 'different_program');
assert.equal(crossProgramPlan.program, 'Fall SHSAT');
delete fields['#bankExamReusePolicy'];
delete fields['#bankExamProgram'];

fields['#bankExamType'] = { value: 'english' };
fields['#mixReadingMinPercent'] = { value: '50' };
fields['#mixReadingMaxPercent'] = { value: '75' };
fields['#mixReadingCount'].value = '';
bankFilteredCache = null;
const rangedReadingPlan = buildQuestionMixDraft();
assert.equal(rangedReadingPlan.complete, true);
assert.equal(rangedReadingPlan.readingMode, 'range');
assert.equal(rangedReadingPlan.readingMinTarget, 2);
assert.equal(rangedReadingPlan.readingMaxTarget, 3);
assert.equal(rangedReadingPlan.readingCount, 2);
assert.equal(rangedReadingPlan.readingTarget, 2);
assert.equal(rangedReadingPlan.subject, 'English / Reading');
assert.equal(rangedReadingPlan.selected.every((question) => inferBankQuestionSubject(question) === 'English / Reading'), true);
delete fields['#mixReadingMinPercent'];
delete fields['#mixReadingMaxPercent'];
delete fields['#bankExamType'];
