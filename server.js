...
function makeQuestionsFromCsv(text, options = {}) {
  ...
  rows.slice(1).forEach((row, index) => {
    const prompt = cleanCsvText(getCsvValue(row, headers, ["Question", "Prompt", "Question Text"]));
    const correctLetter = cleanCsvText(getCsvValue(row, headers, ["Correct Answer Letter", "Answer Letter", "Answer"])).toUpperCase();
    const correctText = cleanCsvText(getCsvValue(row, headers, ["Correct Answer Text", "Answer Text", "Grid-In Answer"]));
    if (!prompt && !correctLetter && !correctText) return;

    const questionType = cleanCsvText(getCsvValue(row, headers, ["Question Type", "Type"])).toLowerCase();
    const isNumeric = /grid|numeric|free/.test(questionType) || correctLetter === "GRID-IN";
    const choiceLines = [];
    const choices = [];

    for (let choiceIndex = 1; choiceIndex <= 8; choiceIndex += 1) {
      const label = cleanCsvText(getCsvValue(row, headers, [`Choice ${choiceIndex} Label`, `Choice ${choiceIndex}`])).toUpperCase();
      const textValue = cleanCsvText(getCsvValue(row, headers, [`Choice ${choiceIndex} Text`, `${label} Text`]));
    ...
    const imageValue = cleanCsvText(getCsvValue(row, headers, ["Question Image", "Question Image ", "Image", "Image URL"]));
    const originalNumber = Number.parseInt(cleanCsvText(getCsvValue(row, headers, ["Question #", "Question Number", "Number"])), 10) || index + 1;
    const testClass = cleanCsvText(getCsvValue(row, headers, ["Test / Class", "Test Class", "Class", "Course", "Exam Category", "Bank Category"]));
    const section = cleanCsvText(getCsvValue(row, headers, ["Section / Topic", "Section", "Topic"])) || "Section 1";
    const skill = cleanCsvText(getCsvValue(row, headers, ["Skill", "Domain", "Standard"]));
    const difficulty = Number.parseInt(cleanCsvText(getCsvValue(row, headers, ["Difficulty (1-4)", "Difficulty", "Level"])), 10) || "";
    const systemNumber = startNumber + questions.length;
    const sourceQuestionId = makeSystemQuestionId(idPrefix, systemNumber);
    const passageId = cleanCsvText(getCsvValue(row, headers, ["Passage ID", "Group", "Group ID"]));
    const passageTitle = cleanCsvText(getCsvValue(row, headers, ["Passage Title", "Stimulus Title", "Title"]));
    const passageText = cleanCsvText(getCsvValue(row, headers, ["Passage / Stimulus", "Passage", "Stimulus", "Reading Passage"]));
