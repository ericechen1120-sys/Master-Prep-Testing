...
// Keep the normalized database state hot between admin actions. Ten seconds was
// short enough that ordinary form work repeatedly downloaded and normalized the
// full JSONB record from PostgreSQL. Five minutes still permits multi-instance
// refreshes while keeping a normal admin session on the fast in-memory path.
const DATA_CACHE_TTL_MS = Math.max(1000, Number.parseInt(process.env.TOPWAY_DATA_CACHE_TTL_MS, 10) || 300000);
...
function sparseSectionChange(section, nextItems, previousJson) {
  if (!previousJson || !["exams", "questionBank"].includes(section) || !Array.isArray(nextItems)) return null;
  let previousItems;
  try {
    previousItems = JSON.parse(previousJson);
  } catch {
    return null;
  }
  if (!Array.isArray(previousItems)) return null;
  const nextIds = nextItems.map((item) => String(item?.id || ""));
  const previousIds = previousItems.map((item) => String(item?.id || ""));
  if (nextIds.some((id) => !id) || previousIds.some((id) => !id)) return null;
  if (new Set(nextIds).size !== nextIds.length || new Set(previousIds).size !== previousIds.length) return null;

  const previousById = new Map(previousItems.map((item) => [String(item.id), JSON.stringify(item)]));
  const nextIdSet = new Set(nextIds);
  const changedItems = nextItems.filter((item) => previousById.get(String(item.id)) !== JSON.stringify(item));
  const removedCount = previousIds.filter((id) => !nextIdSet.has(id)).length;
  const orderChanged = nextIds.join("\u0000") !== previousIds.join("\u0000");
  const changeCount = changedItems.length + removedCount + (orderChanged ? 1 : 0);
  const sparseLimit = Math.max(12, Math.ceil(nextItems.length * 0.2));
  if (changeCount > sparseLimit) return null;
  return { nextIds, changedItems };
}
...
function getDashboardBankClass(question) {
...
function buildAdminDashboardState(data) {
  const bankSummary = new Map();
  (data.questionBank || []).forEach((question) => {
    const label = getDashboardBankClass(question);
    bankSummary.set(label, (bankSummary.get(label) || 0) + 1);
  });
  return {
    exams: (data.exams || []).map(({ questions, ...exam }) => ({
      ...exam,
    ...
    questionBank: [],
    questionBankCount: (data.questionBank || []).length,
    questionBankSummary: [...bankSummary.entries()].map(([label, count]) => ({ label, count })),
    examUsage: (data.exams || []).map((exam) => ({
      id: exam.id,
      title: exam.title,
      code: exam.code,
      ...
      questions: (exam.questions || []).map((question) => ({ sourceGroupId: getQuestionSourceGroupId(question) })),
    })),
    loadedSections: ["submissions", "students", "attempts", "classes", "examUsage"],
  };
}
  ...
  try {
    ...
    if (pool) {
      ...
      changedKeys.forEach((key) => {
          const sparseChange = sparseSectionChange(key, data[key], cachedSectionJson?.[key]);
          if (sparseChange) {
            values.push(sparseChange.nextIds);
            const idsParameter = values.length;
            values.push(JSON.stringify(sparseChange.changedItems));
            const changedParameter = values.length;
            dataExpression = `jsonb_set(${dataExpression}, '{${key}}', (
              SELECT COALESCE(
                jsonb_agg(COALESCE(changed.item, current.item) ORDER BY desired.position)
                  FILTER (WHERE COALESCE(changed.item, current.item) IS NOT NULL),
                '[]'::jsonb
              )
              FROM unnest($${idsParameter}::text[]) WITH ORDINALITY AS desired(id, position)
              LEFT JOIN jsonb_array_elements(COALESCE(data->'${key}', '[]'::jsonb)) AS current(item)
                ON current.item->>'id' = desired.id
              LEFT JOIN jsonb_array_elements($${changedParameter}::jsonb) AS changed(item)
                ON changed.item->>'id' = desired.id
            ), true)`;
          } else {
...
function sendStateChanges(res, changes = {}, extra = {}) {
  return sendJson(res, 200, { stateChanges: changes, ...extra });
}

function replaceStateSections(data, sectionNames = []) {
  return Object.fromEntries(
    sectionNames.map((section) => [section, { replace: Array.isArray(data?.[section]) ? data[section] : [] }])
  );
}

function upsertStateItems(section, items, options = {}) {
  return {
    [section]: {
      upsert: (Array.isArray(items) ? items : [items]).filter(Boolean),
      ...(options.prepend === false ? { prepend: false } : {}),
      ...(Array.isArray(options.removeIds) && options.removeIds.length ? { removeIds: options.removeIds } : {}),
    },
  };
}
  ...
  try {
    if (req.method === "POST" && url.pathname === "/api/admin/login") {
      ...
      if (body.demo === true && isDemoAdminHost(req)) {
        ...
        sessions.add(token);
        const state = body.includeState === true ? buildAdminDashboardState(await loadData()) : undefined;
        return sendJson(res, 200, { token, demo: true, ...(state ? { state } : {}) });
      }
      if (body.password !== ADMIN_PASSWORD) return sendJson(res, 401, { error: "Admin access denied." });
      const token = crypto.randomBytes(24).toString("hex");
      sessions.add(token);
      const state = body.includeState === true ? buildAdminDashboardState(await loadData()) : undefined;
      return sendJson(res, 200, { token, ...(state ? { state } : {}) });
    }
    ...
    if (req.method === "GET" && url.pathname === "/api/admin/state") {
      return sendJson(res, 200, url.searchParams.get("mode") === "dashboard" ? buildAdminDashboardState(data) : data);
    }

    if (req.method === "GET" && url.pathname === "/api/admin/state/sections") {
      const requested = String(url.searchParams.get("names") || "")
        .split(",")
        .map((name) => name.trim())
        .filter((name) => ["exams", "questionBank", "examUsage"].includes(name));
      const names = [...new Set(requested)];
      if (!names.length) return sendJson(res, 400, { error: "Choose at least one admin data section." });
      return sendJson(res, 200, {
        sections: Object.fromEntries(
          names.map((name) => [name, name === "examUsage" ? buildAdminDashboardState(data).examUsage : data[name] || []])
        ),
        loadedSections: names,
      });
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/classes") {
      ...
      await saveData(data);
      return sendStateChanges(res, upsertStateItems("classes", data.classes[0]));
    }
    ...
    if (req.method === "POST" && updateClassMatch) {
      ...
      await saveData(data);
      return sendStateChanges(res, upsertStateItems("classes", classRecord));
    }
    ...
    if (req.method === "DELETE" && deleteClassMatch) {
      ...
      await saveData(data);
      return sendStateChanges(res, {
        classes: { removeIds: [deletedClass.id] },
        ...replaceStateSections(data, ["students"]),
      });
    }
    ...
    if (req.method === "POST" && linkClassStudentMatch) {
      ...
      await saveData(data);
      return sendStateChanges(res, {
        ...upsertStateItems("classes", classRecord),
        ...replaceStateSections(data, ["students", "submissions", "attempts"]),
      });
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/students") {
      ...
      await saveData(data);
      return sendStateChanges(res, {
        ...upsertStateItems("students", student),
        ...replaceStateSections(data, ["submissions", "attempts"]),
      });
    }
    ...
    if (req.method === "POST" && studentLogMatch) {
      ...
      student.updatedAt = new Date().toISOString();
      await saveData(data);
      return sendStateChanges(res, upsertStateItems("students", student));
    }
    ...
    if (req.method === "DELETE" && deleteStudentMatch) {
      ...
      await saveData(data);
      return sendStateChanges(res, {
        students: { removeIds: [deletedStudent.id] },
        ...replaceStateSections(data, ["submissions"]),
      });
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/question-bank/regenerate-ids") {
      ...
      await saveData(data);
      return sendStateChanges(res, upsertStateItems("questionBank", questions));
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/question-bank/create-exam") {
      ...
      await saveData(data);
      return sendStateChanges(res, upsertStateItems("exams", exam), {
        examBuild: {
    ...
    if (req.method === "DELETE" && deleteExamMatch) {
      ...
      await saveData(data);
      return sendStateChanges(res, {
        exams: { removeIds: [deletedExam.id] },
        ...replaceStateSections(data, ["submissions", "attempts"]),
      });
    }
    ...
    if (req.method === "DELETE" && clearAttemptsMatch) {
      ...
      await saveData(data);
      return sendStateChanges(res, replaceStateSections(data, ["attempts"]));
    }
    ...
    if (req.method === "DELETE" && deleteAttemptMatch) {
      ...
      await saveData(data);
      return sendStateChanges(res, { attempts: { removeIds: [attempt.id] } });
    }
    ...
    if (req.method === "DELETE" && deleteSubmissionMatch) {
      ...
      await saveData(data);
      return sendStateChanges(res, {
        submissions: { removeIds: [submission.id] },
        ...replaceStateSections(data, ["attempts"]),
      });
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/storage/old-reports") {
      ...
      await saveData(data);
      return sendStateChanges(res, replaceStateSections(data, ["submissions", "attempts"]), {
...
if (require.main === module) {
  server.listen(PORT, HOST, () => {
    ...
    // Start the database read while the user is opening the login page instead
    // of making the first successful login wait for the full JSONB record.
    loadData().catch((error) => console.error("Topway data preload failed:", error.message));
  });
}

module.exports = {
  normalizeData,
  buildAdminDashboardState,
  sparseSectionChange,
  getPassageUnitKey,
