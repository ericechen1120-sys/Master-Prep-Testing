...
const QUESTION_TYPES = new Set(["multiple", "numeric", "fill_blank", "dropdown", "drag_drop", "table_grid"]);
...
function isOptionQuestion(type) {
  return ["dropdown", "drag_drop", "table_grid"].includes(normalizeQuestionType(type));
}
...
function normalizeAnswerForStorage(value, type) {
  ...
  return ["multiple", "numeric"].includes(normalizeQuestionType(type)) ? answer.toUpperCase() : answer;
}
...
function normalizeAiFormatCandidate(value, allowedTypes) {
  ...
  if (type === "drag_drop") {
    ...
    if (choices.length < 2 || dragTargets.length < 2 || !choices.every((item) => dragTargets.includes(mapping[item]))) return null;
  }
  if (type === "table_grid") {
    const mapping = parseAiJson(answer);
    if (choices.length < 2 || gridRows.length < 2 || !gridRows.every((_, index) => choices.includes(mapping[String(index + 1)]))) return null;
  }
...
async function generateAiFormatCandidates(sourceQuestion, requestedTypes) {
  ...
  const input = {
    ...
    rules: [
      ...
      "For dropdown: questionText must contain exactly one [[blank]], choices must include the correct answer, and answer is the correct option text.",
      "For drag_drop: choices are draggable items, dragTargets are at least two categories, and answerMapping maps every draggable item to one target.",
      "For table_grid: choices are column labels, gridRows are at least two statements, and answerMapping maps row numbers 1, 2, etc. to one column label.",
...
function makeQuestionsFromCsv(text, options = {}) {
  ...
  rows.slice(1).forEach((row, index) => {
    ...
    questions.push({
      ...
      type,
      section,
      choices: ["numeric", "fill_blank"].includes(type) ? [] : typeChoices,
      dragTargets,
      gridRows,
...
function scoreSubmission(exam, answers) {
  const rows = [...exam.questions]
    ...
    .map((question) => {
      ...
      const correctAnswers = (["drag_drop", "table_grid"].includes(normalizeQuestionType(question.type)) ? [question.answer] : splitAcceptedAnswers(question.answer))
...
async function handleApi(req, res) {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/question-bank/ai-format-drafts/save") {
      ...
      await saveData(data);
      return sendJson(res, 200, { data, question });
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/question-bank/create-exam") {
      ...
      const shsatOriginalScoring = body.shsatOriginalScoring === true || body.shsatOriginalScoring === "true";
      if (shsatOriginalScoring && selectedQuestions.length !== 50) {
        return sendJson(res, 400, { error: "The Original SHSAT conversion chart can be used only with exactly 50 questions." });
      }
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/exams") {
      ...
      if (shsatOriginalScoring && finalQuestionCount !== 50) {
        return sendJson(res, 400, { error: "The Original SHSAT conversion chart can be used only with exactly 50 questions." });
      }
      const adaptive = body.adaptive === true || body.adaptive === "true";
      const examType = body.examType === "math" ? "math" : "english";
    ...
    if (req.method === "POST" && updateExamMatch) {
      ...
      if (duplicate) return sendJson(res, 400, { error: "Another exam with the same subject already uses that test code." });
      const shsatOriginalScoring = body.shsatOriginalScoring === true || body.shsatOriginalScoring === "true";
      if (shsatOriginalScoring && (exam.questions || []).length !== 50) {
        return sendJson(res, 400, { error: "The Original SHSAT conversion chart can be used only with exactly 50 questions." });
      }
