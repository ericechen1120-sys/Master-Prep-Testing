...
function normalizeData(data) {
  ...
  normalized.exams.forEach((exam) => fillSharedPassageContent(exam.questions || [], canonicalPassages).forEach(applyKnownQuestionFix));
...
function fillSharedPassageContent(questions, externalCanonicalPassages = null) {
...
function sortExamQuestionsInSourceOrder(questions) {
  return [...(questions || [])]
    .sort((left, right) =>
      String(left.testClass || "").localeCompare(String(right.testClass || "")) ||
      (Number.parseInt(left.originalNumber, 10) || Number.parseInt(left.number, 10) || 0) -
        (Number.parseInt(right.originalNumber, 10) || Number.parseInt(right.number, 10) || 0) ||
      (Number.parseInt(left.number, 10) || 0) - (Number.parseInt(right.number, 10) || 0)
    )
...
async function handleApi(req, res) {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/question-bank/create-exam") {
      ...
      const expandedQuestions = expandCompletePassageSelection(data.questionBank, requestedQuestions);
      const { selected: sourceSelectedQuestions, omitted: omittedVariants } = selectOneQuestionPerSourceGroup(expandedQuestions);
      ...
      return sendJson(res, 200, {
        ...
        examBuild: {
          ...
          expandedPassageCount: expandedQuestions.length - requestedQuestions.length,
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/exams/import-backup") {
      ...
      fillSharedPassageContent(questions, buildCanonicalPassagesBySection(data.questionBank));
      const exam = {
        id: uid("exam"),
