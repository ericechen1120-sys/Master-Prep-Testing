...
const AI_FORMAT_TYPES = new Set(["fill_blank", "dropdown", "drag_drop", "table_grid"]);
...
function cleanAiText(value, max = 8000) {
  return String(value || "").replace(/\0/g, "").trim().slice(0, max);
}

function cleanAiList(value, max = 16) {
  return [...new Set((Array.isArray(value) ? value : []).map((item) => cleanAiText(item, 800)).filter(Boolean))].slice(0, max);
}

function normalizeAiFormatCandidate(value, allowedTypes) {
  const type = normalizeQuestionType(value?.type);
  if (!AI_FORMAT_TYPES.has(type) || !allowedTypes.has(type)) return null;
  const questionText = cleanAiText(value?.questionText);
  const explanation = cleanAiText(value?.explanation, 3000);
  const choices = cleanAiList(value?.choices);
  const dragTargets = cleanAiList(value?.dragTargets);
  const gridRows = cleanAiList(value?.gridRows);
  let answer = "";
  if (["drag_drop", "table_grid"].includes(type)) {
    const mapping = value?.answerMapping && typeof value.answerMapping === "object" && !Array.isArray(value.answerMapping) ? value.answerMapping : {};
    answer = JSON.stringify(Object.fromEntries(Object.entries(mapping).map(([key, item]) => [cleanAiText(key, 300), cleanAiText(item, 300)]).filter(([key, item]) => key && item)));
  } else {
    answer = cleanAiText(value?.answer, 800);
  }
  if (!questionText || !answer) return null;
  if (type === "dropdown" && (!choices.length || !choices.includes(answer) || !questionText.includes("[[blank]]"))) return null;
  if (type === "drag_drop") {
    const mapping = parseAiJson(answer);
    if (choices.length < 3 || dragTargets.length < 2 || !choices.every((item) => dragTargets.includes(mapping[item])) || new Set(Object.values(mapping)).size < 2) return null;
  }
  if (type === "table_grid") {
    const mapping = parseAiJson(answer);
    const hasWeakLabel = choices.some((label) => /^(best answer|not best answer|correct|incorrect|true|false|yes|no|[a-e])$/i.test(label));
    if (choices.length < 2 || gridRows.length < 2 || hasWeakLabel || !gridRows.every((_, index) => choices.includes(mapping[String(index + 1)])) || new Set(Object.values(mapping)).size < 2) return null;
  }
  return { type, questionText, choices, dragTargets, gridRows, answer, explanation };
}

async function generateAiFormatCandidates(sourceQuestion, requestedTypes) {
  if (!OPENAI_API_KEY) throw new Error("OpenAI is not configured on this server. Add OPENAI_API_KEY in Render, then deploy the updated app.");
  const input = {
    task: "Convert one verified multiple-choice practice question into safe teacher-review drafts for the requested interaction types.",
    rules: [
      "Preserve the original academic skill and correct reasoning. Do not invent facts, passages, claims, or answer keys.",
      "Return only valid JSON, with no Markdown, matching { candidates: [...] }.",
      "Each candidate must have type, questionText, choices, dragTargets, gridRows, answer, answerMapping, and explanation.",
      "For fill_blank: write a concise prompt and exact text answer.",
      "For dropdown: use only for vocabulary, grammar, or a mathematical value that fits naturally in exactly one [[blank]]. Choices must include the correct answer and answer is the correct option text.",
      "For drag_drop: use only when the source naturally supports a real sorting task. Provide at least 3 draggable items, at least 2 named categories, map every item, and use more than one category. Never make a drag task from a single MCQ just by rewording it.",
      "For table_grid: use only when there are at least 2 independent statements to classify against evidence. Choices must be meaningful labels such as Supports the claim, Contradicts the claim, or Not enough information — never Correct/Incorrect, Best answer, option letters, or a disguised MCQ. Use more than one label in answerMapping.",
      "If a requested type cannot be made accurately from this source, omit it rather than guessing.",
    ],
    requestedTypes,
    sourceQuestion: {
      id: sourceQuestion.sourceQuestionId || sourceQuestion.id,
      ...
      passageTitle: sourceQuestion.passageTitle,
      passageText: sourceQuestion.passageText,
    },
  };
  const response = await fetch("https://api.openai.com/v1/responses", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${OPENAI_API_KEY}` },
    body: JSON.stringify({ model: OPENAI_MODEL, input: JSON.stringify(input) }),
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(`OpenAI could not generate a draft (${response.status}). ${cleanAiText(payload?.error?.message, 240) || "Please check the API key, billing, and model access in Render."}`);
  let parsed;
  try {
    parsed = parseAiJson(getOpenAiOutputText(payload));
  } catch (_) {
    throw new Error("OpenAI returned a draft that could not be reviewed safely. Please try again.");
  }
  const allowedTypes = new Set(requestedTypes);
  return (Array.isArray(parsed?.candidates) ? parsed.candidates : []).map((candidate) => normalizeAiFormatCandidate(candidate, allowedTypes)).filter(Boolean);
}
...
function normalizeData(data) {
  const normalized = {
    exams: Array.isArray(data?.exams) ? data.exams.map((exam) => ({ ...exam, scoringMode: shsatOriginalScoringMode(exam) })) : [],
  ...
  normalized.questionBank.forEach(applyKnownQuestionFix);
  normalized.exams.forEach((exam) => (exam.questions || []).forEach(applyKnownQuestionFix));
  normalized.submissions.forEach((submission) => {
    const student = normalized.students.find(
      (item) => item.id === submission.studentRecordId || submissionMatchesStudent(submission, item)
    );
...
function selectOneQuestionPerSourceGroup(questions) {
  ...
  [...questions]
    .sort((left, right) => {
      ...
      return leftGenerated - rightGenerated || bankQuestionSort(left, right);
    })
    .forEach((question) => {
...
function sendJson(res, status, data) {
  ...
  if (res.topwayAcceptsGzip && Buffer.byteLength(payload) > 1024) {
    ...
    res.end(zlib.gzipSync(payload, { level: 6 }));
    return;
  }
  res.writeHead(status, headers);
  res.end(payload);
...
async function handleApi(req, res) {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/question-bank/ai-format-drafts") {
      const body = await readBody(req);
      const questionIds = [...new Set((Array.isArray(body.questionIds) ? body.questionIds : []).map(String))].slice(0, 12);
      const requestedTypes = [...new Set((Array.isArray(body.types) ? body.types : []).map(normalizeQuestionType))].filter((type) => AI_FORMAT_TYPES.has(type)).slice(0, 4);
      if (!questionIds.length) return sendJson(res, 400, { error: "Check one or more multiple-choice bank questions first." });
      if (!requestedTypes.length) return sendJson(res, 400, { error: "Choose at least one new question format." });
      const sourceQuestions = data.questionBank.filter((question) => questionIds.includes(question.id));
      if (!sourceQuestions.length) return sendJson(res, 400, { error: "The checked bank questions were not found. Refresh the Bank and try again." });
      const nonMcq = sourceQuestions.filter((question) => normalizeQuestionType(question.type) !== "multiple");
      if (nonMcq.length) return sendJson(res, 400, { error: "AI Formats only converts original multiple-choice questions. Uncheck the other question types and try again." });
      const candidates = [];
      for (const question of sourceQuestions) {
        const drafts = await generateAiFormatCandidates(question, requestedTypes);
        drafts.forEach((candidate) => candidates.push({ sourceQuestionId: question.id, sourceSystemId: question.sourceQuestionId || question.id, candidate }));
      }
      return sendJson(res, 200, { candidates, generatedAt: new Date().toISOString(), model: OPENAI_MODEL });
    }

    if (req.method === "POST" && url.pathname === "/api/admin/question-bank/ai-format-drafts/save") {
      const body = await readBody(req);
      const source = data.questionBank.find((question) => question.id === String(body.sourceQuestionId || ""));
      if (!source) return sendJson(res, 400, { error: "The original bank question was not found. Refresh the Bank and try again." });
      if (normalizeQuestionType(source.type) !== "multiple") return sendJson(res, 400, { error: "Only a saved multiple-choice question can be used as an AI conversion source." });
      const candidate = normalizeAiFormatCandidate(body.candidate, AI_FORMAT_TYPES);
      if (!candidate) return sendJson(res, 400, { error: "This AI draft is incomplete. Generate it again or edit it manually before saving." });
      const bankNumber = nextBankQuestionNumber(data.questionBank);
      const question = makeBankQuestion(
        {
          id: uid("q"),
          number: bankNumber,
          originalNumber: source.originalNumber || source.number || bankNumber,
          testClass: String(source.testClass || "").trim(),
          subject: inferQuestionSubject(source),
          type: candidate.type,
          section: String(source.section || "Section 1").trim() || "Section 1",
          choices: candidate.choices,
          dragTargets: candidate.dragTargets,
          gridRows: candidate.gridRows,
          answer: candidate.answer,
          imageUrl: String(source.imageUrl || "").trim(),
          sharedImageUrl: String(source.sharedImageUrl || "").trim(),
          questionText: candidate.questionText,
          groupId: String(source.groupId || "").trim(),
          passageTitle: String(source.passageTitle || "").trim(),
          passageText: String(source.passageText || "").trim(),
          questionFont: String(source.questionFont || "").trim(),
          points: 1,
          skill: String(source.skill || "").trim(),
          difficulty: Number.parseInt(source.difficulty, 10) || "",
          explanation: candidate.explanation || String(source.explanation || "").trim(),
          sourceQuestionId: makeSystemQuestionId("AI", bankNumber),
          aiGenerated: true,
          aiSourceQuestionId: String(source.sourceQuestionId || source.id),
          sourceGroupId: String(source.sourceGroupId || source.aiSourceQuestionId || source.sourceQuestionId || source.id),
          reviewStatus: "draft",
        },
        bankNumber
      );
    ...
    if (req.method === "POST" && url.pathname === "/api/admin/question-bank/create-exam") {
      ...
      const { selected: sourceSelectedQuestions, omitted: omittedVariants } = selectOneQuestionPerSourceGroup(requestedQuestions);
