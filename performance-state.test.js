...
const { buildAdminDashboardState, sparseSectionChange } = require("../server");
...
const dashboardState = buildAdminDashboardState({
  exams: [{ id: "exam-1", title: "Large Exam", questions: [{ id: "q-1", questionText: "Private detailed question" }] }],
  submissions: [],
  students: [],
  attempts: [],
  classes: [],
  questionBank: [{ id: "bank-1", testClass: "SHSAT ELA", questionText: "Large bank question" }],
});
assert.equal(dashboardState.exams[0].questionCount, 1);
assert.equal(Object.hasOwn(dashboardState.exams[0], "questions"), false);
assert.equal(dashboardState.questionBank.length, 0);
assert.equal(dashboardState.questionBankCount, 1);
assert.equal(dashboardState.examUsage[0].questions[0].sourceGroupId, "q-1");
assert.deepEqual(dashboardState.loadedSections, ["submissions", "students", "attempts", "classes", "examUsage"]);

const appPath = path.join(__dirname, "..", "app.js");
const appSource = fs.readFileSync(appPath, "utf8").replace(/\nrender\(\);\s*$/, "");
assert.match(appSource, /<div class="mix-optional-formats mix-format-picker">/);
assert.doesNotMatch(appSource, /<details class="mix-optional-formats">/);
